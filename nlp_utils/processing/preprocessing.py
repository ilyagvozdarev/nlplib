...
class TOKENIZERS:
    ...
    python_str_split = str.split


class NORMALIZERS:
    def pymorph_normalizer():
        import pymorphy3
        pymorphy3_normalizer = pymorphy3.MorphAnalyzer()
        def normalizer(word):
            return pymorphy3_normalizer.parse(word)[0].normal_form
        return normalizer
...
def preprocess_text(
    ...
    normalizer = None,
    to_lower: bool = True,
    tokenizer = TOKENIZERS.word_tokenize(),
    punctuation = PUNCTUATIONS.string_punctuation,
    join_to_text = False
) -> list[str]:
    '''
        предобработка текста:
        - удаление пунктуации (если задан punctuation)
        - lower case (если задан to_lower)
        - токенизация (токенизатором заданным в tokenizer)
        - удаление стоп-слов (если задан stop_words)
        - нормализация токенов (если задан normalizer)
        - соединение токенов в текст по пробелу (если задан join_to_text)

        Returns
        -------
        список токенов или текст из соединенных по пробелу токенов (если задан join_to_text)
    '''
    processed_tokens = []
    ...
    return processed_tokens if not join_to_text else " ".join(processed_tokens)



# preprocess_text(
#   text,
#   punctuation = PUNCTUATIONS.punct_v1,
#   to_lower = True,
#   stop_words = None,
#   tokenizer = TOKENIZERS.python_str_split,
#   normalizer = None,
#   join_to_text = True
# )
...
def normalize_text(text: str) -> str:
  '''
    нормализация текста:
        - lower case
        - удаление начальных и конечных пробельных символов (strip)
        - удаление пунктуации из каждого слова, если строка состояла только из знаков пунтуации то оставляем
          у нее только 1 знак (в порядке удаления PUNCTUATIONS)
        - разделяем на слова по пробельному символу и соединяем после удаления пунктуации по пробелу " "
  '''
  PUNCTUATIONS = [",", ".", "_", "?", "!", "-", '"', "'"]
