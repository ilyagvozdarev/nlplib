...
def roc_pr_metrics(targets, probas):
    fpr, tpr, thresh = metrics.roc_curve(targets, probas)
    ...
    print(metrics.roc_auc_score(targets, probas))

    precision, recall, threshs = metrics.precision_recall_curve(targets, probas)

    plt.plot(threshs, precision[:-1], label='precision')
    plt.plot(threshs, recall[:-1], label='recall')
    plt.xlabel("threshold")
    plt.legend()