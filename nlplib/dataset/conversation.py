...
A class with methods for handling conversations,
each role has its own message template
'''
...
class Conversation:
    ...
    def apply_template_message(self, message, incomplete_last_bot_message=False):
        '''
        apply a template to the message according to the role
        '''
    ...
    def count_tokens(self, tokenizer, current_messages):
        '''
        number of tokens = sum of tokens in tokenized templated messages
        '''
        final_text = ""
    ...
    def shrink(self, tokenizer, messages, max_tokens):
        '''
        Reduce the number of messages until the number of conversation tokens is less than max_tokens.
        If the first message is a system - keep it, then delete two messages at a time (return user-bot).
        '''
        system_message = []
    ...
    def get_prompt(self, tokenizer=None, max_tokens: int = None, add_suffix_bot: bool = True, incomplete_last_bot_message: bool = False):
        ...
        incomplete_last_bot_message: 
            whether to apply the self.bot_message_template_incomplete template to the last message with the bot role
        add_suffix_bot: 
            whether to append self.bot_suffix to the end of the prompt if the last message has the bot role
    
        Notes
        -----
        self.global_prefix is prepended to the templated prompt
        '''
    ...
    def expand(self, messages, role_mapping = None):
        '''
        append the new messages to the current ones (self.messages).
        if the first message being added has the system role, then remove all current messages

        Parameters
        ----------
        role_mapping: mapping of roles from messages
        '''
        if not role_mapping:
            role_mapping = dict()
