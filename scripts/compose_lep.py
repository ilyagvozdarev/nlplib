...
from nlplib.adapt.lep.lep import make_lep
from nlplib.load_model import (
    check_if_adapt, 
    save_split_adapter,
    load_adapt_and_merge,
    load_model
)
import gc
...
def load_donor_model(model_path, out_dir, alpha_scale=1.0, not_scale_lm_head=False, device="cuda:0"):
    # differences from load_model_base_or_adapt:
    # the adapter is applied only for embeddings (embed_path), since for
    # the projection only the donor's embeddings are used
    adapters_path = None
